"use client";
import React from 'react';
import { Orbitron, Open_Sans } from 'next/font/google';
import Image1 from '@/svg/Embeeded-Security-Image.svg';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faLinkedin, faGithubSquare, faGoogle } from '@fortawesome/free-brands-svg-icons';

const orbitfont = Orbitron({
  subsets: ['latin'],
  weight: ['400', '700'],
  variable: '--font-orbit',
});

const openSans = Open_Sans({
  subsets: ['latin'],
  weight: ['400', '700'],
});

export default function Contact() {
  return (
    <div className={orbitfont.variable}>
      {/* MAIN HERO BOX */}
      <div 
        id="contact"
        className="
          relative z-10 
          md:top-0
          flex flex-col md:flex-row justify-center 
          h-auto md:h-[35rem]
          px-4 w-full md:w-3/5 mx-auto
          bg-[rgb(13,17,23)] bg-opacity-30
          border border-gray-700/50 rounded-2xl backdrop-blur-Bg
          md:border-none md:rounded-none
          py-10 md:py-0
          bg-none md:bg-[url('/images/celestial-fog-small_1.png')]
          md:bg-no-repeat md:bg-[right_2rem_top] md:bg-[length:300px_250px]
        "
      >
        {/* TOP IMAGE — only on desktop */}
        <div className={`${openSans.className} relative mt-4 md:mt-8 flex justify-center md:block`}>
          <Image1 className="hidden md:block w-20 h-20 rounded-full md:left-[-9rem] md:top-[-5rem] relative" />
        </div>

        {/* TEXT SECTION */}
        <div className={`
          ${openSans.className} 
          w-full md:w-1/2
          relative top-4 md:top-12
          left-0 md:left-[-5rem]
          text-center md:text-left
          px-2
        `}>
          <h1 className="text-2xl md:text-3xl" style={{ color: 'rgb(147 154 255)' }}>
            Contact me
          </h1>

          <p className="mt-2 text-base md:text-lg">
            I love sharing ideas, working on new projects, and exploring ways technology can make a difference.{' '}
            <span style={{ color: 'rgb(179 132 249)' }}>
              If my work resonates with you, I’d be happy to hear from you.
            </span>
          </p>

          {/* SOCIAL ICONS */}
          <div className="flex flex-row justify-center md:justify-start space-x-5 mt-5">
            <FontAwesomeIcon icon={faLinkedin} className="text-4xl" style={{ color: 'rgb(179 132 249)' }} />
            <FontAwesomeIcon icon={faGithubSquare} className="text-4xl" style={{ color: 'rgb(179 132 249)' }} />
            <FontAwesomeIcon icon={faGoogle} className="text-4xl" style={{ color: 'rgb(179 132 249)' }} />
          </div>
        </div>
      </div>

      {/* BOTTOM SVG — only on desktop */}
      <div className="flex justify-center">
        <Image1 className="hidden md:block w-11/12 md:w-3/5 h-auto md:h-[35rem] relative md:top-[-10rem] mx-auto z-10" />
      </div>
    </div>
  );
}
